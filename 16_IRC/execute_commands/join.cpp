...
bool    JOIN_command(t_serv *server, const string& args, int sender_fd)
{
    ...
    if (args[0] != '#') {
        ...
        if (it != server->channels.end()) {
            myChannel = it->second;
            if (myChannel->get_is_invite_only() == true && myChannel->is_invited(sender_fd) == false) {
            ...
            myChannel->increment_limits();
        }
        else {
            Channel *new_channel = new Channel(channel_name, sender_fd);
            if (!key.empty()) {
                new_channel->set_key_channel(key);
                new_channel->set_channel_has_key(true);
            }
