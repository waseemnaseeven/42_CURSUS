...
/*   Updated: 2023/06/18 09:10:21 by aaudevar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* SPRITES */
# define C3PO "./textures/c3p0.xpm"
# define R2D2 "./textures/r2d2.xpm"
# define STORM "./textures/storm.xpm"

/* BONUS TEXTURES */
# define PLAFOND "./textures/redstars.xpm"
...
typedef struct s_bots
{
	...
	int		n_bots;
	int		*s_direction;
	int32_t	buffer[SCREENHEIGHT][SCREENWIDTH];
	float	distance;
	int		index;
	float	sprite[2];
	float	inv_det;
...
typedef struct s_sprites
{
	void	*troop_img;
	char	*troop_addr;
	int		troop_bits_per_pixel;
	int		troop_line_length;
	int		troop_endian;
...
void			check_char_helper(t_data *data, int *count,
					float i, float j);
...
void			check_map_doors(t_data *data);

/* sprites */

void			draw_sprites(t_data *data, t_bots *bots, t_sprites *sprites);
